...
class FunctionComponent(ResilientComponent):
    """Component that implements Resilient function 'cb_retrieve_autoruns"""
    ...
    @function("cb_retrieve_autoruns")
    def _cb_retrieve_autoruns_function(self, event, *args, **kwargs):
        ...
        try:
            ...
            while timeouts <= MAX_TIMEOUTS:  # Max timeouts before aborting
                ...
                try:
                    ...
                    integration_server_path = os.path.normpath('/home/integrations/temp-cb-store/' + str(time.mktime(time.gmtime())))
                    if not os.path.exists(integration_server_path): os.makedirs(integration_server_path)
                    ...
                    session.create_process(r'powershell.exe -noexit "Get-CimInstance -ClassName Win32_StartupCommand | Select-Object Name, command, Location, User | Export-Csv "C:\Windows\CarbonBlack\Reports\autoruns.csv" -notypeinformation"', True, None, None, 300, True)
                    with tempfile.NamedTemporaryFile(delete=False) as temp_file:  # Create temporary temp_file for CSV
                        try:
                            ...
                            self.rest_client().post_attachment('/incidents/{0}/attachments'.format(incident_id), temp_file.name, '{0}-autoruns.csv'.format(sensor.hostname))  # Post temp_file to incident
