...
#   @params -> integer: incident_id, string: hostname, string: scan_type (optional)
...
class FunctionComponent(ResilientComponent):
    """Component that implements Resilient function 'cb_run_av_scan"""
    ...
    def _cb_run_av_scan_function(self, event, *args, **kwargs):
        ...
        try:
            ...
            if scan_type is None: scan_type = 'full'  # Default to a full scan
                ...
                try:
                    ...
                    if len(av_paths) > 1:  # If both Defender and Microsoft Security Client were found
                        for process in session.list_processes():  # Use whichever version is running on the endpoint
                            if r'c:\program files\microsoft security client' in process['path'].lower():
                                av_paths.remove(r'C:\Program Files\Windows Defender\mpcmdrun.exe')
                                break

                            elif r'c:\program files\windows defender' in process['path'].lower():
                                av_paths.remove(r'C:\Program Files\Microsoft Security Client\mpcmdrun.exe')
                                break
