...
# Pulled from the "Advanced Indicator of Compromise (IOC) extractor" from github.com/InQuest/python-iocextract by InQuest.
# File: ioc_extractor.py
# Date: 04/17/2018 - Modified: 09/18/2019
# Authors: InQuest (Original), Jared F (editor)

"""Module implementation"""
#   @module -> ioc_extractor
#   @public functions ->
#       extract_iocs(data, refang=False, strip=False)
#       extract_urls(data, refang=False, strip=False)
#       extract_ips(data, refang=False)
#       extract_ipv4s(data, refang=False)
#       extract_ipv6s(data, refang=False)
#       extract_emails(data, refang=False)
#       extract_hashes(data)
...
#       extract_custom_iocs(data, regex_list)
#       refang_email(email)
#       refang_url(url)
#       refang_ipv4(ip_address)
#       defang(ioc)
#   @params -> string: data, bool: refang, bool: strip, string: regex_list, string: url, string: ip_address
...
# Get likely URLs with some unclean/garbage results
#   Credit to Daveo via stackoverflow.com/a/3809435
ADVANCED_UNCLEAN_URL_RE = re.compile(r"""[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)""", re.IGNORECASE | re.VERBOSE | re.UNICODE)
...
# Capture IPv6 addresses.
#   Credit to David M. Syzdek and Qix via stackoverflow.com/a/17871737
IPV6_RE = re.compile(r"""(?:^|(?<=\s))(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))(?=\s|$)""", re.IGNORECASE | re.VERBOSE)
...
def extract_urls(data, refang=False, strip=False):
    ...
    :rtype: :py:func:`itertools.chain`
    """
    unencoded_urls = list(extract_unencoded_urls(data, refang=refang, strip=strip))
    encoded_urls = list(extract_encoded_urls(data, unencoded_urls=unencoded_urls, refang=refang, strip=strip))
...
def extract_unencoded_urls(data, refang=False, strip=False):
    ...
    for url in unencoded_urls:
        url = _refang_common(url.group(1))

        if _is_legit_url(url):
            if refang:
                url = refang_url(url)
            if strip:
                url = re.split(URL_SPLIT_STR, url)[0]

            if url in urls: continue  # Ensures duplicates are not returned
            urls.append(url)

    for url in urls: yield url


def extract_encoded_urls(data, unencoded_urls=None, refang=False, strip=False):
    ...
    for url in HEXENCODED_URL_RE.finditer(data):
        ...
        if refang and (url not in urls) and (_refang_common(url) not in unencoded_urls) and refang_url(_refang_common(url)) not in unencoded_urls:
            url = binascii.unhexlify(url).decode('utf-8')  # Convert and decode hexadecimal.
    ...
    for url in B64ENCODED_URL_RE.finditer(data):
        url = ''.join(url.group(1).split())  # Strip whitespace.

        if len(url) % 4:
            url = url[:-(len(url) % 4)]  # Truncate the string if it's not a multiple of 3 bytes long. We don't care about the end of the string since it's probably garbage.

        if refang and (url not in urls) and (_refang_common(url) not in unencoded_urls) and refang_url(_refang_common(url)) not in unencoded_urls:

            url = base64.b64decode(url).decode('utf-8', 'replace')  # Decode base64.

            url = url[re.search('[hHfF]', url).start():]  # The only valid starts are "http" or "ftp", so look for h/f case insensitive. Remove the first 1-2 bytes if we got back extra characters.

            url = url.split(u'\ufffd')[0].split()[0]  # Stop at the first whitespace or non-unicode character. 8/6/2019 -> We stripped non-unicode already, but leaving this legacy code.

        urls.append(url)

    for url in ADVANCED_UNCLEAN_URL_RE.finditer(data):
        url = url.group(0)

        if 'http' in url or 'ftp' in url:
            if re.search('[hHfF]', url):  # The only valid starts are "http" or "ftp", so look for h/f case insensitive.
                url = url[re.search('[hHfF]', url).start():]  # Remove the first 1-2 bytes if we got back extra characters.
    ...
    for url in urls:
        url = _refang_common(url)

        if _is_legit_url(url):

            if url in cleaned_urls: continue  # Ensure duplicates are not returned.
            ...
            if (not unencoded_urls and url) or (unencoded_urls and (not any(url.replace(urlparse(url).scheme + '://', '') in unencoded_url for unencoded_url in unencoded_urls))):
                yield url
...
def extract_emails(data, refang=False):
    ...
    for email in EMAIL_RE.finditer(_refang_common(data, True)):
        if refang:
...
def _is_legit_url(url):
    """Returns when a URL is legitimate. Helper function to filter out improper/garbage results.

        :param ioc: String URL.
        :rtype: bool
        """
    file_types = ('.txt', '.pdf', '.doc' ,'.docx', '.pps', '.ppt', '.pptx', '.xsl','.xls', '.xlsx', '.csv', '.zip', '.rar', '.7z', '.bin', '.exe', '.js', '.ps1', '.vbs', '.jpg', '.jpeg', '.png', '.gif', '.mp4')
    url = _refang_common(url)
    if ('.' in url) and (not url.endswith('.')) and (not url.replace('.', '').isdigit()) and ('@' not in url) and (not (url.endswith(file_types) and url.count('.') <= 1)) and (not len(url.rsplit('.', 1)[-1]) <= 1):
        return True
    return False


def _refang_common(ioc, bulk_data=False):
    ...
    :rtype: str
    """
    ioc = ioc.replace('[dot]', '.').replace('(dot)', '.').replace('[.]', '.').replace('[:]', ':').replace('[://]', '://').replace('[//]', '//').replace('[/]', '/').replace('(', '').replace(')', '').replace(',', '.')
    if not bulk_data: ioc = ioc.replace(' ', '').replace(u'\u30fb', '.')
    return ioc
...
def defang(ioc):
    ...
    try:
        parsed = urlparse(ioc)
        if parsed.netloc:
            parsed = parsed._replace(netloc=parsed.netloc.replace('.', '[.]'), scheme=parsed.scheme.replace('t', 'x'))
            return parsed.geturl()
    except ValueError:
        pass
...
def main():
    ...
    data = unicodedata.normalize("NFKD", data.decode('utf-8', 'ignore')).encode('ascii', 'ignore')
    data = data.replace(' [:] ', ':').replace(' [.] ', '[.]').replace(' (.) ', '(.)').replace('\\', '/').lower()  # Additional input data cleaning
